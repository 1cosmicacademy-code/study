# Superlativ — Form of Comparison (Superlativ)

[🔙 Back to Grammar Overview](../index.md)

---

## 📺 Instructional Video
An instructional video explaining this grammar topic with practical examples.

<div class="video-container" data-video-id="SYWra2Qzch4"></div>


## 🎯 Lesson Goals

- Form superlative adjectives and adverbs
- Learn the regular **-ste** and irregular **-sten** patterns
- Avoid common pitfalls when using the superlative

---

## 📚 Core Rule

1. **Short adjectives** (usually ending in a consonant) add **-ste** after the adjective:
   - *fast* → *am **fast**sten* (**the fastest**)
2. If the adjective ends in **-e**, **-el**, **-er**, or a vowel‑heavy ending, drop the ending and add **-sten**:
   - *groß* → *am **größ**ten* (**the biggest**)
   - *teuer* → *am **teuer**sten* (**the most expensive**)
3. For adverbs the superlative is built with **am … -sten** as well:
   - *gut* → *am **gut**sten* (**the best**)

> ⚠️ **Common Error:** Adding an extra *e* (e.g., *am schnellest*) is wrong. The correct form is *am **schnell**sten*.

| Adjective | Comparative | Superlative |
|-----------|------------|------------|
| klein | kleiner | am **klein**sten |
| laut | lauter | am **laut**sten |
| teuer | teurer | am **teuer**sten |

---

## ✅ Examples

- **Das schnellste Auto gewinnt das Rennen.** *(The fastest car wins the race.)*
- **Er ist am freundlichsten von allen.** *(He is the friendliest of all.)*
- **Das ist am interessantesten.** *(That is the most interesting.)*


---

---

## 📖 Related Lessons

<div class="lesson-links">

<a href="/a2/kursbuch/lektion-09/" class="lesson-btn">🔤 Lektion 9: Große und kleine Dinge — Big and Small Things</a>
<a href="/a2/ubungsbuch/lektion-09/" class="lesson-btn">🔤 Exercises Lektion 9: Große und kleine Dinge (Big and Small Things)</a>

</div>
---

## 📝 Interactive Exercises

✏️ **Fill in the blank with the correct word:** Write only one word in each field.

<div class="exercise" data-answers='["am besten", "am teuersten", "am kleinsten"]'>
1. Write the superlative of *gut*: ______
2. Turn *teuer* into the superlative: ______
3. Write the superlative of *klein*: ______
</div>

✏️ **Fill in the blank with the correct word:** Write only one word in each field.

<div class="exercise" data-answers='["am lautesten", "am schnellsten", "am freundlichsten"]'>
1. Superlative of *laut*: ______
2. Superlative of *schnell*: ______
3. Superlative of *freundlich*: ______
</div>

