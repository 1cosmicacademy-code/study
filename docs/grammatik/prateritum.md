# زمن الماضي البسيط (Prateritum) في اللغة الألمانية

🎯 **Prateritum** هو زمن الماضي المستخدم في **الكتابة والسرد** (القصص، الروايات، التقارير الإخبارية). في المحادثة اليومية، الألمان يستخدمون Perfekt أكثر.

---

## 📺 فيديو توضيحي
فيديو تعليمي يشرح هذه القاعدة بشكل مبسط مع أمثلة تطبيقية.

<div class="video-container" data-video-id="0vjE-XopATY"></div>


## 💡 الفرق بين Prateritum و Perfekt

| Prateritum | Perfekt |
|-----------|---------|
| 📖 **للنصوص المكتوبة** (قصص، أخبار، روايات) | 🗣️ **للمحادثة اليومية** |
| فعل واحد في الجملة | فعلان: haben/sein + Partizip II |
| كان، كتب، ذهب | قد كان، قد كتب، قد ذهب |

```
Prateritum:  Ich ging nach Hause. (ذهبت إلى البيت)
Perfekt:     Ich bin nach Hause gegangen. (ذهبت إلى البيت)
```

⚠️ **استثناء:** الأفعال المساعدة (sein, haben, werden) والأفعال الناقصة (konnen, mussen...) تستخدم Prateritum حتى في المحادثة اليومية!

```
ألماني يقول: Ich war gestern krank.  ✅
(Prateritum من sein)
لا يقول:    Ich bin gestern krank gewesen. ❌ (نادر وغير طبيعي)
```

---

## ⚠️ تصريف الأفعال المساعدة في Prateritum

### 1. sein (يكون)

| الضمير | sein |
|--------|------|
| ich | **war** |
| du | **warst** |
| er/sie/es | **war** |
| wir | **waren** |
| ihr | **wart** |
| sie/Sie | **waren** |

**أمثلة:**
- `Ich **war** in Berlin.` — كنت في برلين.
- `Wir **waren** im Kino.` — كنا في السينما.

### 2. haben (يملك)

| الضمير | haben |
|--------|-------|
| ich | **hatte** |
| du | **hattest** |
| er/sie/es | **hatte** |
| wir | **hatten** |
| ihr | **hattet** |
| sie/Sie | **hatten** |

**أمثلة:**
- `Ich **hatte** einen Hund.` — كان عندي كلب.
- `Er **hatte** keine Zeit.` — لم يكن عنده وقت.

### 3. werden (يصبح)

| الضمير | werden |
|--------|--------|
| ich | **wurde** |
| du | **wurdest** |
| er/sie/es | **wurde** |
| wir | **wurden** |
| ihr | **wurdet** |
| sie/Sie | **wurden** |

**أمثلة:**
- `Das Wetter **wurde** kalt.` — أصبح الطقس بارداً.
- `Er **wurde** Arzt.` — أصبح طبيباً.

---

## ⚠️ تصريف الأفعال الناقصة (Modalverben) في Prateritum

### 1. konnen (يستطيع)

| ich | du | er/sie/es | wir | ihr | sie/Sie |
|-----|-----|-----------|-----|-----|---------|
| **konnte** | konntest | konnte | konnten | konntet | konnten |

### 2. mussen (يجب / مضطر)

| ich | du | er/sie/es | wir | ihr | sie/Sie |
|-----|-----|-----------|-----|-----|---------|
| **musste** | musstest | musste | mussten | musstet | mussten |

### 3. durfen (يسمح)

| ich | du | er/sie/es | wir | ihr | sie/Sie |
|-----|-----|-----------|-----|-----|---------|
| **durfte** | durftest | durfte | durften | durftet | durften |

### 4. sollen (يجب / من المفترض)

| ich | du | er/sie/es | wir | ihr | sie/Sie |
|-----|-----|-----------|-----|-----|---------|
| **sollte** | solltest | sollte | sollten | solltet | sollten |

### 5. wollen (يريد)

| ich | du | er/sie/es | wir | ihr | sie/Sie |
|-----|-----|-----------|-----|-----|---------|
| **wollte** | wolltest | wollte | wollten | wolltet | wollten |

### 6. mogen / mochte (يحب / يرغب)

| ich | du | er/sie/es | wir | ihr | sie/Sie |
|-----|-----|-----------|-----|-----|---------|
| **mochte** | mochtest | mochte | mochten | mochtet | mochten |

---

## 🎯 قاعدة ذهبية

```
Prateritum = زمن القصص والأخبار
أفعال sein, haben, werden والأفعال الناقصة:
     تستخدم Prateritum حتى في المحادثة!
لأي فعل آخر في الكلام: استخدم Perfekt
```

---

## 📚 الأفعال المنتظمة في Prateritum

للأفعال المنتظمة (schwache Verben)، نضيف **-te** بين الجذر والنهاية الشخصية:

```
مثال: machen → ich machte, du machtest, er machte...
       kaufen → ich kaufte, du kauftest, er kaufte...
       wohnen → ich wohnte, du wohntest, er wohnte...
```

| الضمير | machen (يفعل) | kaufen (يشتري) |
|--------|---------------|----------------|
| ich | **machte** | **kaufte** |
| du | **machtest** | **kauftest** |
| er/sie/es | **machte** | **kaufte** |
| wir | **machten** | **kauften** |
| ihr | **machtet** | **kaufte** |
| sie/Sie | **machten** | **kauften** |

---

## ✅ Präteritum في القصص — الأفعال الشاذة

| المصدر | Prateritum (er) | المعنى |
|--------|-----------------|--------|
| gehen | **ging** | ذهب |
| kommen | **kam** | جاء |
| fahren | **fuhr** | قاد / ذهب |
| sehen | **sah** | رأى |
| essen | **aB** | أكل |
| trinken | **trank** | شرب |
| schreiben | **schrieb** | كتب |
| lesen | **las** | قرأ |
| sprechen | **sprach** | تكلم |
| finden | **fand** | وجد |
| geben | **gab** | أعطى |
| nehmen | **nahm** | أخذ |

---

## ✅ جمل أمثلة مع الترجمة

| الجملة | الترجمة |
|--------|---------|
| Es **war** einmal ein Konig. | كان يا ما كان في قديم الزمان ملك. |
| Ich **hatte** gestern keine Zeit. | لم يكن عندي وقت أمس. |
| Er **wurde** plotzlich reich. | أصبح فجأة غنياً. |
| Wir **konnten** nicht kommen. | لم نستطع المجيء. |
| Sie **musste** viel arbeiten. | كان يجب عليها العمل كثيراً. |
| Er **sah** einen Vogel und **ging** zum Fenster. | رأى عصفوراً وذهب إلى النافذة. |
| **Las** du das Buch? | هل قرأت الكتاب؟ |

---

## 📝 تمارين

✏️ **أكمل الفراغ بالكلمة الصحيحة:** اكتب كلمة واحدة فقط في كل خانة.

<div class="exercise" markdown="1" data-answers='["war","hatten","wurde","warst","hatte"]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Ich ___ gestern krank. (sein) | |
| 2 | Wir ___ einen schönen Tag. (haben) | |
| 3 | Das Wetter ___ kalt. (werden) | |
| 4 | Du ___ im Urlaub. (sein) | |
| 5 | Sie ___ keine Lust. (haben) | |
</div>
✏️ **أكمل الفراغ:** اكتب الإجابة كاملة في الخانة.

<div class="exercise" markdown="1" data-answers='["konnte","musste","durften (durftet? لا — wir durften)","wollte","solltest"]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Ich ___ nicht kommen. (konnen) | |
| 2 | Er ___ viel lernen. (mussen) | |
| 3 | Wir ___ ins Kino gehen. (durfen) | |
| 4 | Sie ___ das Buch lesen. (wollen) | |
| 5 | Du ___ mir helfen. (sollen) | |
</div>
✏️ **رتّب الكلمات لتكوين جملة صحيحة:**

<div class="exercise" markdown="1" data-answers='["ging...","las...","fuhren...","backte...","sah...","schlief...ein","lernten..."]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Ich bin nach Hause gegangen. → Ich ___ nach Hause ___. | |
| 2 | Er hat das Buch gelesen. → Er ___ das Buch ___. | |
| 3 | Wir sind nach Berlin gefahren. → Wir ___ nach Berlin ___. | |
| 4 | Sie hat einen Kuchen gebacken. → Sie ___ einen Kuchen ___. | |
| 5 | Ich habe den Film gesehen. → Ich ___ den Film ___. | |
| 6 | Das Kind ist eingeschlafen. → Das Kind ___ ___. | |
| 7 | Sie haben Deutsch gelernt. → Sie ___ Deutsch ___. | |
</div>
✏️ **اختر الإجابة الصحيحة من الخيارات:**

<div class="exercise" markdown="1" data-answers='["bin gegangen (محادثة) أو ging (قصة) — كلاهما صح","war (Prateritum مع sein)","hatten (Prateritum مع haben هو الطبيعي)","fuhr (قصة)","konnte"]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Gestern (gehe / ging / bin gegangen) ich ins Kino. | |
| 2 | Er (war / ist gewesen / hat gehabt) gestern krank. | |
| 3 | Wir (haben / hatten / haben gehabt) keine Zeit. | |
| 4 | Sie (ist gefahren / fuhr / fahrt) nach Hause. (قصة) | |
| 5 | Ich (habe gekonnt / konnte / kann) nicht schlafen. | |
</div>
### تمرين 5: اقرأ النص واملأ الفراغات

**قصة قصيرة — Der kleine Hund**

Es ___ (sein) einmal ein kleiner Hund. Er ___ (haben) einen großen Knochen. Der Hund ___ (gehen) in den Park und ___ (legen) sich hin. Plotzlich ___ (kommen) ein großer Hund. Er ___ (wollen) den Knochen. Der kleine Hund ___ (haben) Angst und ___ (laufen) weg.

---


---

---

## 📖 الدروس المرتبطة

<div class="lesson-links">

<a href="/a1/kursbuch/lektion-08/" class="lesson-btn">🇩🇪 🔤 Lektion 8: Party</a>
<a href="/a1/ubungsbuch/lektion-08/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 8: Party</a>
<a href="/a1/kursbuch/lektion-14/" class="lesson-btn">🇩🇪 🔤 Lektion 14: Krankschreibung</a>
<a href="/a1/ubungsbuch/lektion-14/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 14: Krankschreibung</a>
<a href="/a1/kursbuch/lektion-19/" class="lesson-btn">🇩🇪 🔤 Lektion 19: Reisen in Deutschland</a>
<a href="/a1/ubungsbuch/lektion-19/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 19: Reisen in Deutschland</a>
<a href="/a1/kursbuch/lektion-20/" class="lesson-btn">🇩🇪 🔤 Lektion 20: Feste und Feiern</a>
<a href="/a1/ubungsbuch/lektion-20/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 20: Feste und Feiern</a>
<a href="/a2/kursbuch/lektion-05/" class="lesson-btn">🇩🇪 🔤 Lektion 5: Mein neuer Job</a>
<a href="/a2/ubungsbuch/lektion-05/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 5: Mein neuer Job</a>
<a href="/a2/kursbuch/lektion-20/" class="lesson-btn">🇩🇪 🔤 Lektion 20: Erinnerungen und Wünsche</a>
<a href="/a2/ubungsbuch/lektion-20/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 20: Erinnerungen und Wünsche</a>

</div>
---

💡 **نصيحة:** أسهل طريقة للدخول في Prateritum — ابدأ بـ **war** و **hatte** فقط. هاتين الكلمتين تستخدمهما الألمانية كل يوم. بعدها أضف الأفعال الناقصة (konnte, musste, wollte). للكتابة والسرد فقط تحتاج باقي الأفعال.
