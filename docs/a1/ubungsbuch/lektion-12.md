# 📝 تمارين Lektion 12: Ausbildung — التعليم والتدريب

> 🎯 **الهدف:** اختبر نفسك في مفردات المدرسة والمهن وفعل "sein" و"möchten".  
> 💡 **نصيحة:** حاول تحل التمارين بنفسك أولاً، بعدين شوف الحل!

---

## التمرين 1: املأ الفراغ بالكلمة الصحيحة 🖊️

**اختر الكلمة المناسبة من القائمة:** (Schule — Lehrer — Schüler — Beruf — Prüfung — Note)

<div class="exercise" markdown="1" data-answers='["Schule","Lehrer","Schüler","Beruf","Prüfung","Note"]'>

| # | الجملة | الكلمة |
|---|--------|--------|
| 1 | Ich gehe in die ___ . (مدرسة) | **______** |
| 2 | Der ___ unterrichtet die Kinder. (مدرّس) | **______** |
| 3 | Der ___ lernt in der Klasse. (طالب) | **______** |
| 4 | Was ist dein ___ ? (مهنة) | **______** |
| 5 | Morgen habe ich eine ___ . (امتحان) | **______** |
| 6 | Ich habe eine gute ___ bekommen. (درجة) | **______** |

</div>

---

## التمرين 2: اختر الإجابة الصحيحة 🔘

**اختر أ, ب أو ج:**

<div class="exercise" markdown="1" data-type="choice" data-answers='["A","B","C","A","B","A","C","A"]'>

| # | الجملة | أ | ب | ج |
|---|--------|---|---|---|
| 1 | Ich ___ Schüler. | bin | bist | ist |
| 2 | Du ___ Student. | bin | bist | ist |
| 3 | Er ___ Lehrer von Beruf. | bin | bist | ist |
| 4 | Wir ___ in der Klasse. | sind | seid | bin |
| 5 | Ihr ___ Schülerinnen. | sind | seid | ist |
| 6 | Was ___ du von Beruf? | bist | ist | bin |
| 7 | Sie (هي) ___ Ärztin. | bin | bist | ist |
| 8 | Sie (هم) ___ Studenten. | sind | seid | ist |

</div>

---

## التمرين 3: صِل بين المهنة والصورة / الوصف 🔗

**صِل المهنة الألمانية بالوصف العربي:**

<div class="exercise" markdown="1" data-type="match" data-answers='["ج","ه","أ","و","ب","د","ح","ز"]'>

| # | Deutsch | # | Arabisch |
|---|---------|---|----------|
| 1 | der Arzt | أ | يطبخ الطعام في المطعم |
| 2 | der Lehrer | ب | يبيع الأشياء في المحل |
| 3 | der Koch | ج | يعالج المرضى |
| 4 | der Polizist | د | يقود السيارة |
| 5 | der Verkäufer | ه | يعلّم الطلاب في المدرسة |
| 6 | der Fahrer | و | يحفظ الأمن |
| 7 | der Ingenieur | ز | يخدم الزبائن في المقهى |
| 8 | der Kellner | ح | يبني ويصمم |

</div>

---

## التمرين 4: رتّب الكلمات لتكوين جملة صحيحة 🔄

**رتب الكلمات في كل سطر لتكون جملة ألمانية صحيحة:**

<div class="exercise" markdown="1" data-answers='["Ich bin Schüler.","Was bist du von Beruf?","Meine Mutter ist Lehrerin.","Wir möchten Polizisten werden.","Was möchtest du werden?","Mein Bruder möchte Arzt werden.","Ich gehe in die Schule.","Ich mag Deutsch."]'>

| # | الكلمات | الجملة الصحيحة |
|---|--------|---------------|
| 1 | bin / Ich / Schüler | **__________________** |
| 2 | du / Was / von Beruf / bist | **__________________** |
| 3 | Lehrerin / ist / Meine Mutter | **__________________** |
| 4 | möchten / Wir / Polizisten / werden | **__________________** |
| 5 | du / Was / möchtest / werden | **__________________** |
| 6 | Arzt / möchte / werden / Mein Bruder | **__________________** |
| 7 | in / gehe / die / Ich / Schule | **__________________** |
| 8 | Deutsch / mag / Ich | **__________________** |

</div>

---

## التمرين 5: ترجم إلى الألمانية 🌐

**ترجم الجمل التالية إلى الألمانية:**

<div class="exercise" markdown="1" data-answers='["Ich bin Schüler.","Was bist du von Beruf?","Er ist Lehrer.","Ich möchte Arzt werden.","Was möchtest du werden?","Meine Schwester möchte Lehrerin werden.","Wir sind in der Schule.","Sie ist Ärztin."]'>

| # | العربية | الألمانية |
|---|---------|----------|
| 1 | أنا طالب. | **__________________** |
| 2 | ما مهنتك؟ | **__________________** |
| 3 | هو مدرّس. | **__________________** |
| 4 | أريد أن أصبح طبيباً. | **__________________** |
| 5 | ماذا تريد أن تصبح؟ | **__________________** |
| 6 | اختي تريد أن تصبح مدرّسة. | **__________________** |
| 7 | نحن في المدرسة. | **__________________** |
| 8 | هي طبيبة. | **__________________** |

</div>

---

## التمرين 6: صحح الخطأ ❌🔧

**في كل جملة خطأ واحد. ابحث عنه وصححه:**

<div class="exercise" markdown="1" data-answers='["Ich bin Schüler","Du bist Student","Er ist Lehrer","Wir sind in der Schule","Ich möchte Arzt werden","Was bist du von Beruf?","Sie sind Studenten","richtig"]'>

| # | الجملة الخطأ | التصحيح |
|---|-------------|---------|
| 1 | Ich **bist** Schüler. | **__________________** |
| 2 | Du **bin** Student. | **__________________** |
| 3 | Er **bin** Lehrer. | **__________________** |
| 4 | Wir **bist** in der Schule. | **__________________** |
| 5 | Ich möchte Arzt **sein**. | **__________________** |
| 6 | Was **ist** du von Beruf? | **__________________** |
| 7 | Sie (هم) **ist** Studenten. | **__________________** |
| 8 | Ich **möchte** Lehrer **werden**. (الجملة خطأ؟ لا — إذا كانت صحيحة اكتب "صحيح") | **__________________** |

</div>

---

## التمرين 7: املأ الفراغ بـ möchten 🔤

**أكمل الجمل باستخدام التصريف الصحيح لـ "möchten":**

<div class="exercise" markdown="1" data-answers='["möchte","möchtest","möchte","möchten","möchtet","möchten","möchtest","möchte"]'>

| # | الجملة | الفعل الناقص |
|---|--------|-------------|
| 1 | Ich ___ Arzt werden. (أريد) | **______** |
| 2 | Du ___ Lehrer werden. (تريد) | **______** |
| 3 | Er ___ Ingenieur werden. (يريد) | **______** |
| 4 | Wir ___ Polizisten werden. (نريد) | **______** |
| 5 | Ihr ___ Studenten sein. (تريدون) | **______** |
| 6 | Sie (هم) ___ Kellner werden. (يريدون) | **______** |
| 7 | Was ___ du werden? (تريد) | **______** |
| 8 | Meine Schwester ___ Ärztin werden. (تريد) | **______** |

</div>

---

## التمرين 8: اختر الإجابة الصحيحة (مفردات المدرسة) 🔘

**اختر أ, ب أو ج:**

<div class="exercise" markdown="1" data-type="choice" data-answers='["B","B","A","B","B","B","B","B"]'>

| # | الجملة | أ | ب | ج |
|---|--------|---|---|---|
| 1 | Ich habe heute ___ . (واجب منزلي) | eine Prüfung | eine Hausaufgabe | eine Note |
| 2 | Der ___ unterrichtet Deutsch. | Schüler | Lehrer | Fahrer |
| 3 | In der ___ sind 20 Schüler. | Klasse | Küche | Prüfung |
| 4 | Ich habe eine gute ___ in Mathe. | Prüfung | Note | Schule |
| 5 | Mathe ist mein Lieblings- ___ . (مادتي المفضلة) | Beruf | Fach | Buch |
| 6 | Morgen schreiben wir eine ___ . | Note | Prüfung | Klasse |
| 7 | Ich brauche einen ___ für die Schule. (قلم رصاص) | Kugelschreiber | Bleistift | Heft |
| 8 | Das ___ ist auf dem Tisch. (الكتاب) | Heft | Buch | Bleistift |

</div>

---

## التمرين 9: اقرأ النص وأجب (صح / خطأ) 📖

**اقرأ النص ثم اختر صح (Richtig) أو خطأ (Falsch):**

> Hallo! Ich bin Tom. Ich bin 14 Jahre alt und ich bin Schüler. Ich gehe in die 9. Klasse.
> Meine Lieblingsfächer sind Mathe und Sport. Ich mag Deutsch nicht — Deutsch ist schwer!
>
> Mein Vater ist von Beruf Ingenieur. Meine Mutter ist Ärztin. Sie arbeitet im Krankenhaus.
>
> Was möchte ich werden? Ich möchte Polizist werden. Polizist ist ein cooler Beruf!

<div class="exercise" markdown="1" data-type="choice" data-answers='["A","B","B","A","B","A","A","A"]'>

| # | الجملة | صح | خطأ |
|---|--------|:--:|:---:|
| 1 | Tom ist Schüler. | ○ | ○ |
| 2 | Tom geht in die 10. Klasse. | ○ | ○ |
| 3 | Toms Lieblingsfächer sind Mathe und Deutsch. | ○ | ○ |
| 4 | Tom mag Deutsch nicht. | ○ | ○ |
| 5 | Toms Vater ist Lehrer. | ○ | ○ |
| 6 | Toms Mutter arbeitet im Krankenhaus. | ○ | ○ |
| 7 | Tom möchte Polizist werden. | ○ | ○ |
| 8 | Tom findet Polizist cool. | ○ | ○ |

</div>

---

## التمرين 10: اكتب عن نفسك ✍️

**أكمل الجمل لتحكي عن نفسك بالعربية والألمانية:**

1. Ich heiße **__________________**. (اسمك)
2. Ich bin **__________________** Jahre alt. (عمرك)
3. Ich bin **__________________** . (طالب / طالبة — Schüler / Schülerin)
4. Mein Lieblingsfach ist **__________________** . (مادتك المفضلة)
5. Mein Vater ist **__________________** von Beruf. (مهنة أبوك)
6. Meine Mutter ist **__________________** von Beruf. (مهنة أمك)
7. Ich möchte **__________________** werden. (حلمك المستقبلي)

---

🎉 **أحسنت!** الحين تقدر تتكلم عن المدرسة والمهن والأحلام المستقبلية بالألمانية! أعد التمارين إذا حسيت إنك محتاج مراجعة.

---

## 📚 القواعد المرتبطة

<div class="grammar-links">
  <a href="/grammatik/modalverben/" class="grammar-btn">🇩🇪 Modalverben ⚡</a>
  <a href="/grammatik/satzbau/" class="grammar-btn">🇩🇪 Satzbau 🏗️</a>
  <a href="/grammatik/nebensatze/" class="grammar-btn">🇩🇪 Nebensätze 🔗</a>
  <a href="/grammatik/verbkonjugation/" class="grammar-btn">🇩🇪 Verbkonjugation 🔤</a>
</div>

[🔙 العودة إلى المستوى A1](../index.md) • [📕 كتاب التمارين](index.md)
