# 📝 Exercises Lektion 15: Wege in der Stadt — Ways in the City

> 🎯 **Goal:** Test yourself on city vocabulary, directions, and how to ask for and give directions in German.
> 💡 **Tip:** Try to solve the exercises yourself first, then check the answers!

---

## Exercise 1: Fill in zum or zur 🖊️

**Choose zum or zur:**

<div class="exercise" markdown="1" data-answers='["zum", "zur", "zum", "zur", "zum", "zur", "zum", "zur", "zum", "zur"]'>
| # | Sentence | zum / zur |
|---|----------|-----------|
| 1 | Wie komme ich ___ Bahnhof? | **______** |
| 2 | Wie komme ich ___ Post? | **______** |
| 3 | Wie komme ich ___ Krankenhaus? | **______** |
| 4 | Wie komme ich ___ Schule? | **______** |
| 5 | Wie komme ich ___ Supermarkt? | **______** |
| 6 | Wie komme ich ___ Apotheke? | **______** |
| 7 | Wie komme ich ___ Kino? | **______** |
| 8 | Wie komme ich ___ Bibliothek? | **______** |
| 9 | Wie komme ich ___ Restaurant? | **______** |
| 10 | Wie komme ich ___ Kirche? | **______** |

</div>
---
## Exercise 2: Choose the correct answer 🔘

**Choose a, b or c:**

<div class="exercise" markdown="1" data-type="choice" data-answers='["B", "C", "B", "B", "B", "A", "C", "C", "A", "B"]'>
1. Entschuldigung! Wie komme ich ___ Bahnhof?
   - a) zur
   - b) zum
   - c) zu der

2. Gehen Sie ___. (straight)
   - a) links
   - b) rechts
   - c) geradeaus

3. An der ___ (traffic light) links.
   - a) Kreuzung
   - b) Ampel
   - c) Ecke

4. Wie komme ich ___ Post?
   - a) zum
   - b) zur
   - c) zu den

5. ___ (Then) gehen Sie rechts.
   - a) Aber
   - b) Dann
   - c) Und

6. Es ist nicht ___ (far).
   - a) weit
   - b) kalt
   - c) groß

7. "You're welcome!" in German:
   - a) Danke!
   - b) Entschuldigung!
   - c) Bitte schön!

8. Wo ist ___ Kino?
   - a) der
   - b) die
   - c) das

9. Gehen Sie ___ (at the intersection) rechts.
   - a) an der Kreuzung
   - b) an der Ampel
   - c) um die Ecke

10. Danke! — The reply:
    - a) Tschüss!
    - b) Bitte schön!
    - c) Hallo!

</div>
---
## Exercise 3: Match 🔗

**Match the German sentence with the correct English translation:**

<div class="exercise" markdown="1" data-type="match" data-answers='["E", "A", "B", "C", "D", "G", "F", "J", "H", "I"]'>
| # | Deutsch | # | English |
|---|---------|---|---------|
| 1 | Wie komme ich zum Bahnhof? | A | Go straight, then left. |
| 2 | Gehen Sie geradeaus, dann links. | B | Where is the supermarket? |
| 3 | Wo ist der Supermarkt? | C | The hospital is around the corner. |
| 4 | Das Krankenhaus ist um die Ecke. | D | Right at the traffic light. |
| 5 | An der Ampel rechts. | E | How do I get to the train station? |
| 6 | Es ist nicht weit. | F | The station is on the right. |
| 7 | Der Bahnhof ist rechts. | G | It is not far. |
| 8 | Entschuldigung! | H | Is that far? |
| 9 | Ist das weit? | I | Thank you very much! |
| 10 | Vielen Dank! | J | Excuse me! |

</div>
---
<div class="exercise" markdown="1" data-answers='["Wie komme ich zum Bahnhof?","Gehen Sie geradeaus.","Wo ist die Post?","An der Ampel links.","Das Café ist um die Ecke.","Wie komme ich zur Schule?","Das ist nicht weit.","Gehen Sie dann links."]'>

| # | Words | Correct Sentence |
|---|-------|-----------------|
| 1 | ich / zum / Wie / Bahnhof / komme / ? | |
| 2 | Sie / Gehen / geradeaus / . | |
| 3 | der / Post / Wo / ist / ? | |
| 4 | Ampel / An / links / der / . | |
| 5 | ist / Das / um / Ecke / Café / die / . | |
| 6 | komme / Wie / zur / ich / Schule / ? | |
| 7 | weit / nicht / Das / ist / . | |
| 8 | Sie / dann / Gehen / links / . | |

</div>

---

## Exercise 5: Translate into German 🌐

**Translate these sentences into German:**

<div class="exercise" markdown="1" data-answers='["Wie komme ich zum Supermarkt?", "Wo ist das Krankenhaus?", "Gehen Sie geradeaus.", "Dann rechts.", "An der Ampel links.", "Es ist nicht weit.", "Das Café ist um die Ecke.", "Vielen Dank! — Bitte schön!", "Der Bahnhof ist rechts.", "Ist das weit?"]'>
| # | English | German |
|---|---------|--------|
| 1 | How do I get to the supermarket? | **__________________** |
| 2 | Where is the hospital? | **__________________** |
| 3 | Go straight ahead. | **__________________** |
| 4 | Then right. | **__________________** |
| 5 | Left at the traffic light. | **__________________** |
| 6 | It's not far. | **__________________** |
| 7 | The cafe is around the corner. | **__________________** |
| 8 | Thank you very much! — You're welcome! | **__________________** |
| 9 | The station is on the right. | **__________________** |
| 10 | Is that far? | **__________________** |

</div>
---
## Exercise 6: Find and correct the mistake ❌🔧

**Each sentence has one mistake. Find it and correct it:**

<div class="exercise" markdown="1" data-answers='["zum", "geradeaus", "An der Ampel", "zur", "das", "um die", "die zweite", "Entschuldigung!"]'>
| # | Wrong sentence | Correction |
|---|----------------|------------|
| 1 | Wie komme ich **zur** Bahnhof? | **__________________** |
| 2 | Gehen Sie **rechts** (the meaning should be straight). | **__________________** |
| 3 | **An der Kreuzung** (the meaning should be traffic light). | **__________________** |
| 4 | Wie komme ich **zum** Apotheke? | **__________________** |
| 5 | Wo ist **der** Kino? | **__________________** |
| 6 | Das Café ist **an der** Ecke. | **__________________** |
| 7 | Gehen Sie **die erste** Straße rechts. | **__________________** |
| 8 | **Entschuldigung** (without punctuation mark). | **__________________** |

</div>
---
## Exercise 7: Read the dialogue and complete 📖

**Read the dialogue and complete the sentences:**

> **Anna:** Entschuldigung! Wie komme ich zum Krankenhaus?
> **Herr Müller:** Gehen Sie geradeaus bis zur Ampel. Dann links. Das Krankenhaus ist dann in der zweiten Straße rechts.
> **Anna:** Also: geradeaus bis zur Ampel, dann links, dann die zweite Straße rechts?
> **Herr Müller:** Ja, genau! Das Krankenhaus ist um die Ecke.
> **Anna:** Ist das weit?
> **Herr Müller:** Nein, nicht weit. Fünfzehn Minuten zu Fuß.
> **Anna:** Vielen Dank!
> **Herr Müller:** Gern geschehen!

<div class="exercise" markdown="1" data-answers='["zum Krankenhaus","geradeaus, zur","links","zweiten","nicht weit","fünfzehn","Vielen Dank","Gern geschehen"]'>

| # | Question | Answer |
|---|----------|--------|
| 1 | Anna asks: "Wie komme ich ___ ___?" | |
| 2 | Herr Müller says: "Gehen Sie ___ bis ___ Ampel." | |
| 3 | Then Anna goes ___ (left). | |
| 4 | The hospital is in the ___ (second) street on the right. | |
| 5 | It is ___ (not far). | |
| 6 | You need ___ minutes on foot. | |
| 7 | Anna says at the end: "___ ___!" | |
| 8 | Herr Müller replies: "___ ___!" | |

</div>

---

## Exercise 8: One missing word 🧩

**Each sentence has one missing word. Complete it:**

<div class="exercise" markdown="1" data-answers='["zum", "geradeaus", "Ampel", "Um", "weit", "Entschuldigung", "Vielen", "zweite", "in", "zur"]'>
1. Wie komme ich ___ Bahnhof? (zu + dem)
   → **______**

2. Gehen Sie ___ . (straight)
   → **______**

3. An der ___ links. (traffic light)
   → **______**

4. ___ die Ecke. (around)
   → **______**

5. Es ist nicht ___ . (far)
   → **______**

6. ___ ! (Excuse me)
   → **______**

7. ___ Dank! (many / much)
   → **______**

8. Gehen Sie die ___ Straße rechts. (second)
   → **______**

9. Der Supermarkt ist ___ der Bahnhofstraße. (in)
   → **______**

10. Wie komme ich ___ Post? (zu + der)
    → **______**

</div>
---
## Exercise 9: Read the map and write directions 🗺️

**Read the description: You are standing at "der Markt". Write the directions for each place:**

```
                    ┌─────────────────┐
                    │  die Post       │
                    └─────────────────┘
                         │
    ┌────────────┐       │      ┌────────────┐
    │ die Kirche │───────┼──────│ der Markt  │─── (You are here)
    └────────────┘       │      └────────────┘
                         │
                    ┌────┴────┐
                    │  Ampel  │
                    └────┬────┘
                         │
    ┌────────────┐       │      ┌────────────┐
    │ der Bahnhof│───────┼──────│ das Kino   │
    └────────────┘       │      └────────────┘
                         │
                    ┌────┴────┐
                    │Kreuzung │
                    └────┬────┘
```

**Complete the sentences:**

<div class="exercise" markdown="1" data-answers='["geradeaus", "geradeaus", "geradeaus", "links"]'>
1. From **der Markt** to **die Post**: "Gehen Sie _____, then at the _____ turn left."
2. From **der Markt** to **der Bahnhof**: "Gehen Sie _____, then at the traffic light go _____, then at the intersection turn _____."
3. From **der Markt** to **das Kino**: "Gehen Sie _____, then at the traffic light turn _____, the cinema is then on the _____."
4. From **der Markt** to **die Kirche**: "Gehen Sie _____, the church is on the _____."

</div>
---
## Exercise 10: Write a short dialogue 🎭

**Write a short dialogue in German. One person asks for directions to a place, and another answers. Use these words:**

> 🏪 **Place:** der Supermarkt
> 🚦 **Words you must use:** Entschuldigung, geradeaus, Ampel, links, rechts, nicht weit, Danke

**Complete the dialogue:**

**Person A:** Entschuldigung! Wie komme ich ___ ___ ?

**Person B:** Gehen Sie ___ bis zur ___. Dann ___.

**Person A:** Ist das ___?

**Person B:** Nein, das ist ___. Fünf Minuten.

**Person A:** ___!

**Person B:** ___ ___!

✅ **Example answer (your answer may be different):**

**Person A:** Entschuldigung! Wie komme ich **zum Supermarkt**?
**Person B:** Gehen Sie **geradeaus** bis zur **Ampel**. Dann **links**.
**Person A:** Ist das **weit**?
**Person B:** Nein, das ist **nicht weit**. Fünf Minuten.
**Person A:** **Danke!**
**Person B:** **Bitte schön!**

---

🎉 **Well done!** Now you can ask for and give directions in German! Repeat the exercises if you feel you need more practice.

---

## 📚 Related Grammar

<div class="grammar-links">
  <a href="/grammatik/wechselprapositionen/" class="grammar-btn">🇩🇪 Wechselpräpositionen 🔀</a>
  <a href="/grammatik/dativ/" class="grammar-btn">🇩🇪 Dativ 🎁</a>
  <a href="/grammatik/imperativ/" class="grammar-btn">🇩🇪 Imperativ 📢</a>
  <a href="/grammatik/prapositionen/" class="grammar-btn">🇩🇪 Präpositionen 📍</a>
</div>

[🔙 Back to A1 Level](../index.md) • [📕 Exercise Book](index.md)
