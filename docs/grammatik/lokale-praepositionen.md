# Lokale Präpositionen — حروف الجر المكانية (wo?, wohin?, woher?)

[🔙 العودة إلى القواعد](../index.md)

---

## 📺 فيديو توضيحي
فيديو تعليمي يشرح هذه القاعدة بشكل مبسط مع أمثلة تطبيقية.

<div class="video-container" data-video-id="hHlKmbECKnE"></div>


## 🎯 أهداف الدرس

- فهم كيفية استخدام حروف الجر المكانية في الجمل اليومية.
- تمييز بين الأسئلة عن المكان (wo?), الاتجاه (wohin?) والموضع الأصلي (woher?).
- تطبيق القواعد على أمثلة واقعية وتمارين تفاعلية.

---

## 📚 القاعدة الأساسية

| الحرف الجر | معنى السؤال | مثال السؤال |
|------------|------------|--------------|
| **wo?** | أين؟ (الموقع الثابت) | **Wo** bist du? — أين أنت؟ |
| **wohin?** | إلى أين؟ (الاتجاه) | **Wohin** gehst du? — إلى أين تذهب؟ |
| **woher?** | من أين؟ (المصدر) | **Woher** kommst du? — من أين أتيت؟ |

### ملاحظات شائعة

- **wo?** يستخدم عندما يكون الموقع ثابتًا ولا يتضمن حركة.
- **wohin?** يرافقه فعل يدل على الحركة مثل *gehen, fahren, laufen*.
- **woher?** يستخدم مع أفعال تشير إلى الأصل أو المصدر مثل *kommen, stammen*.
- إذا كان الفعل غير حركي، نستخدم **wo?** فقط.

---

## 📝 أمثلة مع ترجمة

- **Wo** ist das Buch? — أين الكتاب؟ ✅
- **Wohin** fährst du? — إلى أين تسافر؟ ✅
- **Woher** kommt er? — من أين يأتي؟ ✅
- **Wo** liegt die Stadt? — أين تقع المدينة؟ ✅
- **Wohin** willst du gehen? — إلى أين تريد الذهاب؟ ✅


---

---

## 📖 الدروس المرتبطة

<div class="lesson-links">

<a href="/a2/kursbuch/lektion-04/" class="lesson-btn">🇩🇪 🔤 Lektion 4: Wohnen und Nachbarn</a>
<a href="/a2/ubungsbuch/lektion-04/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 4: Wohnen und Nachbarn</a>
<a href="/a2/kursbuch/lektion-06/" class="lesson-btn">🇩🇪 🔤 Lektion 6: Auf dem Weg zur Arbeit</a>
<a href="/a2/ubungsbuch/lektion-06/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 6: Auf dem Weg zur Arbeit</a>
<a href="/a2/kursbuch/lektion-10/" class="lesson-btn">🇩🇪 🔤 Lektion 10: Durch die Stadt</a>
<a href="/a2/ubungsbuch/lektion-10/" class="lesson-btn">🇩🇪 🔤 Übungsbuch Lektion 10: Durch die Stadt</a>

</div>
---

## 🎯 تمارين تفاعلية

✏️ **أكمل الفراغ بالكلمة الصحيحة:** اكتب كلمة واحدة فقط في كل خانة.

<div class="exercise" data-answers='["Wo", "Wohin", "Woher", "Wo", "Wohin"]'>
1. ___? ist das Restaurant? (where)  
2. ___? gehst du nach Hause? (where to)  
3. ___? kommt dein Freund? (where from)  
4. ___? liegt das Museum? (where)  
5. ___? fahren wir morgen? (where to)
</div>

✏️ **أكمل الفراغ بالكلمة الصحيحة:** اكتب كلمة واحدة فقط في كل خانة.

<div class="exercise" data-answers='["Wo", "Wohin", "Woher"]'>
1. ___? steht das Bild? (where)  
2. ___? fliegst du? (where to)  
3. ___? stammt das Rezept? (where from)
</div>

