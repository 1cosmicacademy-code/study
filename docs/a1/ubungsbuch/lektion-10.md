# 📝 تمارين Lektion 10: Bus und Bahn

> 🎯 **الهدف:** اختبر نفسك في وسائل النقل، شراء التذاكر، والسؤال عن الاتجاهات بالألمانية.  
> 💡 **نصيحة:** حاول تحل التمارين بنفسك أولاً، بعدين شوف الحل!

---

## التمرين 1: اختر الإجابة الصحيحة 🚌

**اختر وسيلة النقل المناسبة للجملة:**

<div class="exercise" markdown="1">

| # | وسيلة النقل |
|---|-------------|
| **أ** | **der Bus** — الباص |
| **ب** | **der Zug** — القطار |
| **ج** | **die U-Bahn** — المترو |
| **د** | **das Fahrrad** — الدراجة |
| **هـ** | **das Flugzeug** — الطيارة |
| **و** | **das Schiff** — السفينة |

</div>

<div class="exercise" markdown="1" data-type="choice" data-answers='["أ","ب","ج","هـ","و","د"]'>

| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Ich fahre mit dem ___________ zur Schule. (وسيلة في الشارع) | **____** |
| 2 | Wir fahren mit dem ___________ nach Berlin. (وسيلة سريعة على السكة) | **____** |
| 3 | Sie fährt mit der ___________ in die Stadt. (تحت الأرض) | **____** |
| 4 | Er fährt mit dem ___________ nach Ägypten. (في السماء) | **____** |
| 5 | Ich fahre mit dem ___________ zum Strand. (على الماء) | **____** |
| 6 | Kinder fahren mit dem ___________ zur Schule. (رياضة — دوستين) | **____** |

</div>

---

## التمرين 2: املأ الفراغ — فعل **fahren** 🚗

**صرف فعل **fahren** بين قوسين بشكل صحيح:**

| # | الجملة | التصريف الصحيح |
|---|--------|---------------|
| 1 | Ich ___________ mit dem Bus. (fahren) | **______________** |
| 2 | Du ___________ mit der Bahn. (fahren) | **______________** |
| 3 | Er ___________ mit dem Auto. (fahren) | **______________** |
| 4 | Wir ___________ mit dem Zug. (fahren) | **______________** |
| 5 | Ihr ___________ mit dem Taxi. (fahren) | **______________** |
| 6 | Sie (they) ___________ mit der U-Bahn. (fahren) | **______________** |
| 7 | ___________ du mit dem Fahrrad? (fahren) | **______________** |
| 8 | Meine Mutter ___________ mit dem Bus zur Arbeit. (fahren) | **______________** |

<div class="exercise" markdown="1" data-answers='["fahre","fährst","fährt","fahren","fahrt","fahren","Fährst","fährt"]'>

| # | الجملة | التصريف الصحيح |
|---|--------|---------------|
| 1 | Ich ___________ mit dem Bus. (fahren) | **______________** |
| 2 | Du ___________ mit der Bahn. (fahren) | **______________** |
| 3 | Er ___________ mit dem Auto. (fahren) | **______________** |
| 4 | Wir ___________ mit dem Zug. (fahren) | **______________** |
| 5 | Ihr ___________ mit dem Taxi. (fahren) | **______________** |
| 6 | Sie (they) ___________ mit der U-Bahn. (fahren) | **______________** |
| 7 | ___________ du mit dem Fahrrad? (fahren) | **______________** |
| 8 | Meine Mutter ___________ mit dem Bus zur Arbeit. (fahren) | **______________** |

</div>

---

## التمرين 3: صِل (Match) — التذاكر 🎫

**صِل الجملة الألمانية بالترجمة العربية الصحيحة:**

<div class="exercise" markdown="1" data-type="match" data-answers='["ج","د","أ","ب","و","ه","ز"]' data-options='["أ","ب","ج","د","ه","و","ز"]'>

| الألمانية | العربية | الحرف |
|-----------|---------|:-----:|
| 1. Eine Fahrkarte bitte! | أ. تذكرة اليوم الكامل | |
| 2. Einfach oder hin und zurück? | ب. كم السعر؟ | |
| 3. Die Tageskarte | ج. تذكرة من فضلك! | |
| 4. Was kostet das? | د. اتجاه واحد ولا رايح جاي؟ | |
| 5. Der Automat | ه. كم تستغرق الرحلة؟ | |
| 6. Wie lange dauert die Fahrt? | و. آلة التذاكر | |
| 7. Gute Reise! | ز. رحلة سعيدة! | |

</div>

---

## التمرين 4: املأ الفراغ — **mit** + وسيلة النقل 🚍

**اكمل الجملة بـ **mit** + وسيلة النقل الصحيحة (انتبه لـ Dativ!):**

<div class="exercise" markdown="1" data-answers='["mit dem Bus","mit dem Zug","mit der U-Bahn","mit dem Auto","mit dem Fahrrad","mit dem Flugzeug","mit der S-Bahn","mit dem Taxi"]'>

| # | الجملة | أكمل |
|---|--------|------|
| 1 | Ich fahre ___________ (der Bus) zur Arbeit. | **______________** |
| 2 | Wir fahren ___________ (der Zug) nach Hamburg. | **______________** |
| 3 | Sie kommt ___________ (die U-Bahn). | **______________** |
| 4 | Er fährt ___________ (das Auto) nach Hause. | **______________** |
| 5 | Die Kinder fahren ___________ (das Fahrrad) zur Schule. | **______________** |
| 6 | Ich reise ___________ (das Flugzeug) nach Spanien. | **______________** |
| 7 | Fährst du ___________ (die S-Bahn)? | **______________** |
| 8 | Sie fahren ___________ (das Taxi) zum Hotel. | **______________** |

</div>

---

## التمرين 5: ترجم من العربية إلى الألمانية 🌍

**ترجم هذه الجمل:**

<div class="exercise" markdown="1" data-answers='["Ich möchte eine Fahrkarte nach Berlin, bitte.","Wann fährt der Zug?","Wie lange dauert die Fahrt?","Ich fahre mit dem Bus zur Schule.","Von welchem Gleis?","Gute Reise!","Wo ist der Ausgang?","Was kostet das?"]'>

| # | العربية | الألمانية |
|---|---------|-----------|
| 1 | أريد تذكرة إلى برلين من فضلك. | **______________** |
| 2 | متى يتحرك القطار؟ | **______________** |
| 3 | كم تستغرق الرحلة؟ | **______________** |
| 4 | أذهب إلى المدرسة بالباص. | **______________** |
| 5 | من أي رصيف؟ | **______________** |
| 6 | رحلة سعيدة! | **______________** |
| 7 | وين المخرج؟ | **______________** |
| 8 | كم السعر؟ | **______________** |

</div>

---

## التمرين 6: صحح الخطأ ❌

**في كل جملة خطأ واحد. اكتب الجملة الصحيحة:**

<div class="exercise" markdown="1" data-answers='["Ich fahre mit dem Bus.","Wann fährt der Zug?","Einfach oder hin und zurück?","Wir fahren mit dem Taxi.","Wo ist das Gleis?","Das kostet 5 Euro.","Fahrt ihr mit dem Bus?"]'>

| # | الجملة (فيها خطأ) | الصحيح |
|---|------------------|--------|
| 1 | Ich fahre mit der Bus. | **______________** |
| 2 | Wann fahrt der Zug? | **______________** |
| 3 | Ich möchte eine Fahrkarte nach Berlin. — Einfach und zurück? | **______________** |
| 4 | Wir fährt mit dem Taxi. | **______________** |
| 5 | Wo ist der Gleis? | **______________** |
| 6 | Das kosten 5 Euro. | **______________** |
| 7 | Fährst ihr mit dem Bus? | **______________** |

</div>

---

## التمرين 7: اقرأ جدول مواعيد القطارات 📋

**هذا جدول مواعيد القطارات من محطة Frankfurt إلى Munich. اقرأ ثم اجب على الأسئلة:**

| Zug | Abfahrt (انطلاق) | Gleis | Dauer (المدة) | Preis (السعر) |
|-----|-------------------|-------|---------------|---------------|
| ICE 101 | 08:00 | Gleis 3 | 3 Stunden | 80 € |
| ICE 205 | 10:30 | Gleis 5 | 3 Stunden | 65 € |
| RE 7 | 12:15 | Gleis 1 | 4 Stunden | 40 € |
| ICE 309 | 14:45 | Gleis 2 | 3 Stunden | 90 € |

<div class="exercise" markdown="1" data-answers='["ICE 101 fährt um 8 Uhr.","ICE 205 fährt von Gleis 5.","Die Fahrt dauert 4 Stunden.","ICE 309 kostet 90 Euro.","RE 7 ist am billigsten. Er kostet 40 Euro.","ICE 205 fährt um 10:30."]'>

**الأسئلة:**

| # | السؤال | الإجابة بالألمانية |
|---|--------|-------------------|
| 1 | Wann fährt ICE 101? | **______________** |
| 2 | Von welchem Gleis fährt ICE 205? | **______________** |
| 3 | Wie lange dauert die Fahrt mit RE 7? | **______________** |
| 4 | Was kostet ICE 309? | **______________** |
| 5 | Welcher Zug ist am billigsten (أرخص واحد) ? | **______________** |
| 6 | Welcher Zug fährt um 10:30? | **______________** |

</div>

---

## التمرين 8: رتب الكلمات لتكون جملة 🧩

**رتب الكلمات لتكون جملة صحيحة:**

<div class="exercise" markdown="1" data-answers='["Ich möchte eine Fahrkarte.","Wann fährt der Zug?","Ich fahre mit dem Bus.","Das kostet 60 Euro.","Wie lange dauert die Fahrt?","Wo ist Gleis 5?","Gute Reise!","Fahren Sie mit dem Taxi?"]'>

| # | الكلمات | الجملة الصحيحة |
|---|---------|----------------|
| 1 | Fahrkarte / eine / Ich / möchte | **______________** |
| 2 | fährt / der / Wann / Zug / ? | **______________** |
| 3 | mit / fahre / Bus / dem / Ich | **______________** |
| 4 | kostet / Das / 60 / Euro | **______________** |
| 5 | dauert / Fahrt / Wie / die / lange / ? | **______________** |
| 6 | ist / Wo / Gleis / 5 / ? | **______________** |
| 7 | Reise / Gute / ! | **______________** |
| 8 | dem / Taxi / mit / Sie / Fahren / ? | **______________** |

</div>

---

## التمرين 9: حوار في المحطة — رتب المحادثة 💬

**رتب الجمل بالترتيب الصحيح ليكون حواراً منطقياً (من 1 إلى 7):**

| الترتيب | الجملة |
|:-------:|--------|
| **___** | Ja, wir fahren mit dem Zug nach Stuttgart. |
| **___** | Guten Morgen! Eine Fahrkarte nach Stuttgart, bitte. |
| **___** | Die Fahrt dauert 2 Stunden. |
| **___** | Einfach, bitte. Was kostet das? |
| **___** | Guten Morgen! Einfach oder hin und zurück? |
| **___** | Das kostet 35 Euro. |
| **___** | Und wie lange dauert die Fahrt? |

✅ **الحل:**

| الترتيب | الجملة |
|:-------:|--------|
| **1** | Guten Morgen! Eine Fahrkarte nach Stuttgart, bitte. |
| **2** | Guten Morgen! Einfach oder hin und zurück? |
| **3** | Einfach, bitte. Was kostet das? |
| **4** | Das kostet 35 Euro. |
| **5** | Und wie lange dauert die Fahrt? |
| **6** | Die Fahrt dauert 2 Stunden. |
| **7** | Ja, wir fahren mit dem Zug nach Stuttgart. (السياق: السائل يسأل صديقه) |

---

## التمرين 10: اكتب جملة عن نفسك ✏️

**اكتب جملة ألمانية صحيحة عن وسيلة النقل التي تستخدمها:**

| # | الموقف | جملتك بالألمانية |
|---|--------|-----------------|
| 1 | كيف تذهب إلى المدرسة/الجامعة/العمل؟ | **______________** |
| 2 | كيف تذهب إلى السوق أو المركز التجاري؟ | **______________** |
| 3 | بأي وسيلة سفر تسافر إلى بلد آخر؟ | **______________** |

✅ **أمثلة (نماذج إجابات):**

| # | مثال |
|---|------|
| 1 | **Ich fahre mit dem Bus zur Schule.** أو **Ich fahre mit dem Auto zur Arbeit.** |
| 2 | **Ich fahre mit der U-Bahn zum Markt.** أو **Ich gehe zu Fuß zum Supermarkt.** (مشي) |
| 3 | **Ich reise mit dem Flugzeug nach Deutschland.** أو **Ich fahre mit dem Schiff in die Türkei.** |

> 💡 **جمل إضافية ممكن تستخدمها:**
> - **Ich gehe zu Fuß.** = أنا أمشي (بدون وسيلة نقل)
> - **Ich fahre mit dem Fahrrad.** = أنا أذهب بالدراجة
> - **Ich nehme den Bus.** = أنا أركب الباص (طريقة ثانية للقول)

---

## 📊 جدول تقدمك

| التمرين | الموضوع | ✔️ حلّيته؟ |
|---------|---------|:----------:|
| 1 | اختر وسيلة النقل الصحيحة | ⬜ |
| 2 | تصريف فعل fahren | ⬜ |
| 3 | وصل الجمل (تذاكر) | ⬜ |
| 4 | mit + Dativ لوسائل النقل | ⬜ |
| 5 | ترجمة عربي → ألماني | ⬜ |
| 6 | تصحيح الخطأ | ⬜ |
| 7 | قراءة جدول مواعيد القطارات | ⬜ |
| 8 | ترتيب الكلمات | ⬜ |
| 9 | ترتيب المحادثة | ⬜ |
| 10 | كتابة جمل عن نفسك | ⬜ |

> 🎉 **أحسنت!** إذا حلّيت كل التمارين، فأنت تقدر تشتري تذكرة قطار، تسأل عن الاتجاهات، وتتحدث عن وسائل النقل بالألمانية بثقة!
> 
> 🚍 **تذكر:** في ألمانيا، المواصلات العامة ممتازة. جرب تقول **"Eine Fahrkarte bitte!"** في المرة الجاية!

---

## 📚 القواعد المرتبطة

<div class="grammar-links">
  <a href="/grammatik/wechselprapositionen/" class="grammar-btn">🇩🇪 Wechselpräpositionen (حروف الجر المتغيرة)</a>
  <a href="/grammatik/dativ/" class="grammar-btn">🇩🇪 Dativ (حالة الجر)</a>
  <a href="/grammatik/imperativ/" class="grammar-btn">🇩🇪 Imperativ (صيغة الأمر)</a>
  <a href="/grammatik/prapositionen/" class="grammar-btn">🇩🇪 Präpositionen (حروف الجر)</a>
</div>

[🔙 رجوع إلى التمارين](index.md)
