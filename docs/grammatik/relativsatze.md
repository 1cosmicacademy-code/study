# الجمل الموصولة (Relativsätze) في اللغة الألمانية

🎯 **Relativsätze** (الجمل الموصولة) هي جمل داخل جمل — نستخدمها عشان نعطي **معلومة إضافية** عن شخص أو شيء من غير ما نبدأ جملة جديدة. يعني بدل "عندي صديق. هذا الصديق يعيش في برلين." نقول "عندي صديق **الذي يعيش في برلين**."

---

## 📺 فيديو توضيحي
فيديو تعليمي يشرح هذه القاعدة بشكل مبسط مع أمثلة تطبيقية.

<div class="video-container" data-video-id="w2GFzRi9x1o"></div>


## 💡 أدوات الوصل (Relativpronomen)

في الألمانية، أدوات الوصل هي نفس أدوات التعريف تقريباً: **der, die, das** — بس مع اختلاف بسيط في Dativ و Genitiv.

| الحالة | مذكر (m) | مؤنث (f) | محايد (n) | جمع (pl) |
|--------|----------|----------|-----------|----------|
| **Nominativ** | **der** | **die** | **das** | **die** |
| **Akkusativ** | **den** | **die** | **das** | **die** |
| **Dativ** | **dem** | **der** | **dem** | **denen** |
| **Genitiv** | **dessen** | **deren** | **dessen** | **deren** |

> 📌 **معلومة:** أداة الوصل تأخذ **نفس جنس (Gender) الاسم** اللي تشير إليه، لكن **حالتها (Case)** حسب وظيفتها في الجملة الموصولة.

---

## ⚠️ كيف نحدد أداة الوصل الصحيحة؟

### خطوتين بسيطتين:

1️⃣ **نوع الاسم (مذكر/مؤنث/محايد/جمع)** ← يحدد صف أداة الوصل
2️⃣ **دور الاسم في الجملة الموصولة** ← يحدد الحالة (Nominativ / Akkusativ / Dativ)

### مثال:

> **Der Mann**, **der** hier wohnt, ist Arzt.
> (الرجل الذي يسكن هنا هو طبيب.)

- الاسم: **Der Mann** ← مذكر (m)
- دور "der Mann" في الجملة الموصولة: هو اللي بيسكن ← Nominativ
- إذن: **der** (مذكر + Nominativ)

> **Der Mann**, **den** du siehst, ist mein Bruder.
> (الرجل الذي تراه هو أخي.)

- الاسم: **Der Mann** ← مذكر (m)
- دور "der Mann" في الجملة الموصولة: هو اللي بتشوفه ← Akkusativ (den du siehst = أنت ترى **الرجل**)
- إذن: **den** (مذكر + Akkusativ)

---

## 🎯 ترتيب الكلمات في الجملة الموصولة

📌 **القاعدة المهمة:** في الجملة الموصولة، **الفعل يروح على آخر الجملة**!

```
[جملة رئيسية], [أداة الوصل + ... + الفعل]
```

**أمثلة:**
- Das ist der Film, **den ich gesehen habe**. — هذا الفيلم الذي رأيته.
- Kennst du die Frau, **die dort sitzt**? — هل تعرف المرأة التي تجلس هناك؟
- Das ist der Hund, **der im Garten spielt**. — هذا الكلب الذي يلعب في الحديقة.

---

## ✅ جمل أمثلة لكل حالة

| الحالة | المثال | الترجمة |
|--------|--------|---------|
| **Nominativ (m)** | Der Mann, **der** dort steht, ist mein Vater. | الرجل الذي يقف هناك هو والدي. |
| **Nominativ (f)** | Die Frau, **die** hier wohnt, ist Lehrerin. | المرأة التي تسكن هنا هي معلمة. |
| **Nominativ (n)** | Das Kind, **das** schreit, ist müde. | الطفل الذي يبكي متعب. |
| **Akkusativ (m)** | Der Hund, **den** du siehst, ist süß. | الكلب الذي تراه لطيف. |
| **Akkusativ (f)** | Die Blume, **die** ich kaufe, ist schön. | الزهرة التي أشتريها جميلة. |
| **Dativ (m)** | Der Mann, **dem** ich helfe, ist alt. | الرجل الذي أساعده كبير بالسن. |
| **Dativ (pl)** | Die Freunde, **denen** ich schreibe, wohnen in Berlin. | الأصدقاء الذين أكتب لهم يسكنون في برلين. |
| **Dativ (f)** | Die Frau, **der** ich das Buch gebe, ist meine Mutter. | المرأة التي أعطيها الكتاب هي أمي. |
| **Genitiv (m)** | Der Mann, **dessen** Auto rot ist, fährt schnell. | الرجل الذي سيارته حمراء يقود بسرعة. |
| **Genitiv (pl)** | Die Kinder, **deren** Eltern kommen, sind glücklich. | الأطفال الذين آباؤهم قادمون سعداء. |

---

## 📝 تمارين

✏️ **اختر الإجابة الصحيحة من الخيارات:**

<div class="exercise" markdown="1" data-answers='["der","die","das","den","denen"]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Der Mann, ___ (der / den / dem) dort arbeitet, ist nett. | |
| 2 | Die Frau, ___ (die / der / deren) ich kenne, kommt aus Berlin. | |
| 3 | Das Kind, ___ (das / den / dem) im Garten spielt, ist glücklich. | |
| 4 | Der Hund, ___ (den / dem / der) du fütterst, ist süß. | |
| 5 | Die Freunde, ___ (denen / die / der) ich helfe, sind dankbar. | |
</div>
✏️ **أكمل الفراغ بالكلمة الصحيحة:** اكتب كلمة واحدة فقط في كل خانة.

<div class="exercise" markdown="1" data-answers='["den","die","das","dem","deren"]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Der Film, ___ ich gesehen habe, war langweilig. | |
| 2 | Die Frau, ___ dort sitzt, ist meine Nachbarin. | |
| 3 | Das Buch, ___ auf dem Tisch liegt, gehört mir. | |
| 4 | Der Lehrer, ___ ich die Blume schenke, ist freundlich. | |
| 5 | Die Kinder, ___ Spielzeug kaputt ist, weinen. | |
</div>
✏️ **ترجم الجملة إلى الألمانية:**

<div class="exercise" markdown="1" data-answers='["Ich habe einen Freund, der in Hamburg wohnt.","Das ist ein Film, den ich gestern gesehen habe.","Kennst du die Frau, die im Krankenhaus arbeitet?","Das ist der Hund, dem ich Futter gebe.","Ich lese ein Buch, das sehr interessant ist."]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Ich habe einen Freund. **Der Freund** wohnt in Hamburg. | |
| 2 | Das ist ein Film. **Den Film** habe ich gestern gesehen. | |
| 3 | Kennst du die Frau? **Die Frau** arbeitet im Krankenhaus. | |
| 4 | Das ist der Hund. **Dem Hund** gebe ich Futter. | |
| 5 | Ich lese ein Buch. **Das Buch** ist sehr interessant. | |
</div>
✏️ **ترجم الجملة إلى الألمانية:**

<div class="exercise" markdown="1" data-answers='["Der Mann, der dort arbeitet, ist jung.","Die Frau, die ich sehe, ist schön.","Das Kind, dem ich helfe, ist klein.","Der Film, den ich gesehen habe, war gut.","Die Leute, deren Auto kaputt ist, warten auf Hilfe."]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | Der Mann, **den** dort arbeitet, ist jung. (غلط في الحالة) | |
| 2 | Die Frau, **der** ich sehe, ist schön. (غلط في الحالة) | |
| 3 | Das Kind, **das** ich helfe, ist klein. (غلط في الحالة) | |
| 4 | Der Film, **der** ich gesehen habe, war gut. (غلط) | |
| 5 | Die Leute, **denen** Auto kaputt ist, warten auf Hilfe. (غلط) | |
</div>
✏️ **ترجم الجملة إلى الألمانية:**

<div class="exercise" markdown="1" data-answers='["Der Mann, der hier wohnt, ist Arzt.","Die Frau, die ich sehe, ist schön.","Das Buch, das ich gekauft habe, ist interessant.","Die Kinder, denen ich helfe, sind klein.","Der Freund, dessen Auto blau ist, heißt Ahmed."]'>
| # | الجملة | الإجابة |
|---|--------|---------|
| 1 | الرجل الذي يسكن هنا هو طبيب. | |
| 2 | المرأة التي أراها جميلة. | |
| 3 | الكتاب الذي اشتريته ممتع. | |
| 4 | الأطفال الذين أساعدهم صغار. | |
| 5 | الصديق الذي سيارته زرقاء اسمه أحمد. | |
</div>
---

---

💡 **نصيحة سريعة:** أصعب شيء في Relativsätze هو تحديد الحالة (Case). اسأل نفسك: وش وظيفة الاسم الموصول في الجملة الموصولة؟ 
- هو اللي بيسوي الفعل؟ → Nominativ
- عليه الفعل؟ → Akkusativ
- بعد حرف جر أو فعل يأخذ Dativ؟ → Dativ
- ملكية؟ → Genitiv

🔙 [العودة إلى القواعد](index.md)
